...
export RUN_ID="${RUN_ID:-smoke_seq10l_fp16_sliding_sched20}"
...
export EVAL_BATCH_SEQS="${EVAL_BATCH_SEQS:-256}"
export EVAL_STRIDE="${EVAL_STRIDE:-64}"
export WARMDOWN_ITERS="${WARMDOWN_ITERS:-20}"
...
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" \
  experiments/seq10l_fp16_sliding_sched20/train_gpt.py
