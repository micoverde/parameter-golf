...
class Hyperparameters:
    ...
    # Late-phase STE QAT on exported int6 matrices.
    qat_enable = bool(int(os.environ.get("QAT_ENABLE", "1")))
    qat_bits = int(os.environ.get("QAT_BITS", 6))
    qat_start_scale = float(os.environ.get("QAT_START_SCALE", 0.10))
...
def dequantize_tensor(q: Tensor, s: Tensor, dtype: torch.dtype) -> Tensor:
    if s.ndim > 0:
...
def fake_quantize_tensor_ste(t: Tensor, bits: int = 6) -> Tensor:
    q, s = quantize_float_tensor(t, bits=bits)
    dq = dequantize_tensor(q, s, dtype=t.dtype)
    return t + (dq - t).detach()

def set_qat_modules(module: nn.Module, active: bool, bits: int) -> None:
    for submodule in module.modules():
        if isinstance(submodule, CastedLinear):
            submodule._qat_active = active
            submodule._qat_bits = bits
...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if (
            self.training
            and getattr(self, "_qat_active", False)
            and not getattr(self, "_qat_skip", False)
        ):
            weight = fake_quantize_tensor_ste(weight, bits=getattr(self, "_qat_bits", 6))
...
class GPT(nn.Module):
    def __init__(
        ...
        for i in range(max(num_layers - 2, 0), num_layers):
            self.blocks[i].attn.c_k._qat_skip = True
...
def main() -> None:
    ...
    log0(
        f"qat_enable:{args.qat_enable} qat_bits:{args.qat_bits} "
        f"qat_start_scale:{args.qat_start_scale:.4f}"
    )
    ...
    while True:
        ...
        qat_active = args.qat_enable and scale <= args.qat_start_scale
        set_qat_modules(base_model, qat_active, args.qat_bits)
        if qat_active and not qat_activation_logged:
            log0(f"qat_active:True step:{step} lr_scale:{scale:.6f} bits:{args.qat_bits}")
            qat_activation_logged = True
        zero_grad_all()
